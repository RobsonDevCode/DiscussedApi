...
namespace DiscussedApi.Validations.Comment
{
    public class EditCommentRequestValidation : AbstractValidator<UpdateCommentDto>
    {
        public EditCommentRequestValidation()
        {
            ...
            RuleFor(x => x.Id).NotNull().WithMessage("Comment Id cannot be null when editing comment!");
            RuleFor(x => x.Id).NotNull().WithMessage("Comment Id cannot be empty when editing comment!");
